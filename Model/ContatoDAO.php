...
class ContatoDAO
{
    ...
    public function editarContatoDAO($contato)
    {
        ...
        // Monta a query (edita)
        $sql = "UPDATE contato SET nome = ?, sobrenome = ?, email = ? WHERE id = ?";

        // Cria o Prepared Statement
        $stmt = $conexao->prepare($sql);

        // Vincula o parâmetro que será editado
        $stmt->bind_param("ssss", $nome, $sobrenome, $email, $id);

        // Recebe o ID guardado no objeto
        $id = $contato->id;
